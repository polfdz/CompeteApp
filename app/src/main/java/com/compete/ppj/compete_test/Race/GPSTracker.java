...
public class GPSTracker extends Service implements LocationListener {
    ...
    //Run
    public GPSTracker(Context context, Run _run) {
                ...
                if (isGPSEnabled) {
                    if (location == null) {
                        ...
                        if (locationManager != null) {
                            ...
                            Log.d("GPS Enabled", "lastLocation " + location.toString());
    ...
    public double getSpeed(){
        if(location != null){
            speed = location.getSpeed()*3600/1000; //KM/S
        }
        // return speed
        return speed;
    }
    ...
    public void onLocationChanged(Location _location) {
        ...
        if(lastLoc != null)
        {
            double ttf = (_location.getTime() - lastLoc.getTime()) / 1000;
            int R = 6371;
            ...
            double d = R * c;
            totalDistance += d;
            runMenu.sendDistance(d); //add distance to be incremented
            runMenu.sendAvSpeed(location.getSpeed()); //add avSpeed to be incremented
            runMenu.updateStats("1"); //update "1"--> running
        ...
        if(location!=null){
            ...
            av_speed = (av_speed + location.getSpeed()) / 2;



            runMenu.setLatitude(latitude);
            runMenu.setLongitude(longitude);
            runMenu.setSpeed(speed);
            runMenu.setTotalDistance(totalDistance);
        }
